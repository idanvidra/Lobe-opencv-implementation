...
def run_on_webcam():
    ...
    while True:
        ...
        # Resizing into 224x224 for faster prediction.
        size = (224, 224)
        im = ImageOps.fit(im, size, Image.ANTIALIAS)
        im = im.resize((224,224))
        
        # Print prediction label per image
        # Can and should be converted to some action - e.g. robot movement
        outputs = model.predict(im)['Prediction']
        print(f"Predicted: {outputs} at time: {datetime.now()}")

        # Optional - show camera feed on screen
        cv2.imshow("Camera",frame)

        # Stop run with 'q' keyboard press
